import {Drawer, DrawerContent, DrawerFooter, DrawerHeader, DrawerTrigger} from '@/components/ui/drawer.tsx';
        ...
        <DrawerContent className="bg-white">
          <DrawerHeader className="mb-3 px-[10px]">
            <div className="flex items-center flex-col gap-y-2 justify-between">
              <DialogTitle className="font-bold text-[22px] leading-[130%]">
                {!file ?
                  'Загрузка новой фотографии' : 'Фотография на вашей странице'}
              </DialogTitle>
              <DialogDescription className="text-start text-base leading-[140%] text-primary-foreground text-center">
                {!file ? 'Загрузите свою настоящую фотографию. Вы можете выбрать изображение в формате JPG, WEBP или PNG.' : 'Выбранная область будет показываться на вашей странице.'} </DialogDescription>
            </div>
          </DrawerHeader>
          {!file &&
            <div className="flex flex-col gap-y-2 mb-10 px-2">
              ...
              >
                <p className="max-w-[220px] hidden sm:block text-center">Перетащите файл в эту область, чтобы
                  загрузить</p>
                <div className="size-12 rounded-full bg-primary flex items-center justify-center relative">
					<span
            className="font-bold text-white text-2xl absolute left-[50%] top-[50%] translate-x-[-50%] translate-y-[-60%]">
						&uarr;
					</span>
                ...
                <Button
                  className={'bg-white text-primary-foreground shadow-none sm:font-semibold sm:text-lg'}
                >
                  Выберите файл
                </Button>
              </div>
          ...
          {file &&
            <div className="flex flex-col gap-y-5 px-2 max-w-[340px] px-2 mx-auto ">
              <div
                className="w-full h-[340px] relative after:content-[''] after:block after:absolute after:w-[250px] after:h-[250px] after:top-5 after:left-[50%] after:translate-x-[-50%] after:bg-white/50 after:rounded-full">
                <img src={file.imgUrl} alt={file.file.name} className="w-full h-full object-cover"/>
              </div>
              {apiValidationErrors?.avatar && <FieldErrors error={apiValidationErrors.avatar}/>}
            </div>}
        ...
        >
          <DialogHeader className="flex items-center flex-col gap-y-2 justify-between">
            <DialogTitle className="font-semibold text-[25px] leading-[130%] text-center">
              {!file ?
                'Загрузка новой фотографии' : 'Фотография на вашей странице'} </DialogTitle>
