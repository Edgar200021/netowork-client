...
const formattedDate = `${day}.${month}.${year} г.`;
	...
	return (
		<div className={cn('px-[10px] py-4 rounded-[16px] border-[1px] border-border flex flex-col gap-y-6 w-full md:px-6 md:py-[38px] relative', className)}>
			<div className="flex items-center gap-x-2">
				<img className='w-[50px] h-[50px] object-cover' src={taskReply.freelancer.avatar || defaultProfile} alt={taskReply.freelancer.firstName} />
				<div className="flex flex-col gap-y-1">
					<div className="div flex gap-x-2 items-center">
						<span className='font-semibold'>{taskReply.freelancer.firstName} {taskReply.freelancer.lastName}</span>
			...
			<Button className='md:absolute md:top-[38px] md:right-8' variant="outline">Обсудить условия</Button>
		</div>
	)
}
